...
public class SpatialHashing
{
    ...
    public int totalOccupiedCells = 0; // Tracks the number of occupied cells
    public int totalObjectPairs = 0; // Tracks the total number of object pairs in the hash table
            ...
            for (int y = minKey.y; y <= maxKey.y; y++)
            {
                for (int z = minKey.z; z <= maxKey.z; z++)
                {
                    ...
                    if (!hashTable.ContainsKey(key))
                    {
                        ...
                        totalOccupiedCells++; // Increment occupied cells when a new key is added
                    }
                    hashTable[key].Add(obj);
                    totalObjectPairs++; // Increment the count of object pairs
                }
            }
        }
    }
            ...
            for (int y = minKey.y; y <= maxKey.y; y++)
            {
                for (int z = minKey.z; z <= maxKey.z; z++)
                {
                    ...
                    if (hashTable.ContainsKey(key))
                    {
                        ...
                        totalObjectPairs--; // Decrement the count of object pairs

                        if (hashTable[key].Count == 0)
                        {
                            hashTable.Remove(key);
                            totalOccupiedCells--; // Decrement occupied cells when a key is removed
                        }
                    }
                }
            }
    ...
    {
        hashTable.Clear();
        totalOccupiedCells = 0; // Reset the occupied cells count
        totalObjectPairs = 0; // Reset the total object pairs count
